...
## Purpose
Entry point for a full validation. Runs the 11 skills in order, grouped into five phases (Discovery, Customer & Market, Strategy, Assessment, Synthesis), tracks progress with a checklist, and supports resuming from any step or running a single step. It owns no artifact of its own; it sequences the others and writes everything into the current project directory.

## Key Files

| File | Description |
|------|-------------|
| `SKILL.md` | 229 lines. Step list, copyable progress tracker, full / partial / single-step invocation modes, artifact locations, time estimates (2–3 h total). Tools: `Read Write WebSearch WebFetch Grep Edit` (union of all skills) |
...
- Adding or reordering a skill: edit the ASCII pipeline diagram, the Progress Tracker checklist, the Validation Timeline table, and the Artifact Locations tree, all in `SKILL.md`.
